...
```{r ASR plot, echo=False, results='asis'}
...
Q = matrix(data=c(0,1,10,0), nrow=2, dimnames=list(c(1,2), c(1,2)))
...
```{r ASR_custom_plot, echo=False, results='asis'}
plotTree(mltree, offset=0.5)
tiplabels(pie = to.matrix(adj.NumLegs, sort(unique(adj.NumLegs))), piecol = c("pink", "blue"), cex = 0.3)
...
add.simmap.legend(leg=c("Legless", "Has Legs"), colors=c("pink", "blue"), prompt = FALSE, 
                  x=0.55, y=1.5)
```
This tree actually makes less sense to me as it would seem there are multiple transitions from leglessness, to having legs, and back again even within 2 completely legless clades. I suppose it's possible but it violates our notion of parsimonious tree building, for sure. 
